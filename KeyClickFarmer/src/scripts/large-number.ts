...
const SEPARATE_DIGIT = 5

export class LargeNumber {

    _value: string = "0"
    _intNumbers: Array<number> = new Array<number>()
    _dotPos: number = 0
    ...
    // はじめにインスタンスの値を定義する関数
    define(b: number | string | LargeNumber) {

        // 値の分割
        const bStr = typeof b == "string" ? b : b.toString()
        const dotPosition = bStr.indexOf(".")
        const intPart = bStr.substring(0, dotPosition)
        const decPart = bStr.substring(dotPosition + 1)
        
        // 代入
        this._value = bStr
        for (let i = intPart.length; i > 0; i -= SEPARATE_DIGIT) {
            const digit = Math.min(i, SEPARATE_DIGIT)
            const part = Number(bStr.substr(i - digit, digit))
            this._numbers.push(part)
        }
        this._dotPos = dotPosition
    }

    // 値の和を非破壊的に返す
    static add(lmA: LargeNumber, lmB: LargeNumber): LargeNumber {
        const aMax = lmA._numbers.length
        const bMax = lmB._numbers.length
        const max = Math.max(aMax, bMax)
        let ans = ""
        let up = 0
        for (let i = 0; i < max; i += 1) {
            let a = i < aMax ? lmA._numbers[i] : 0
            let b = i < bMax ? lmB._numbers[i] : 0
            let sum = a + b + up
            ...
            // 文字列に挿入
            ans = sum == 0 ? ans : sum.toString() + ans
        } 
    }
